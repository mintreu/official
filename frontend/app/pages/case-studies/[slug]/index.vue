  ...
  <div class="min-h-screen bg-white dark:bg-gray-950 py-8">
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
      <nav class="flex items-center space-x-2 text-sm">
        ...
        <NuxtLink to="/case-studies" class="text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400">
          Case Studies
        </NuxtLink>
        <Icon name="lucide:chevron-right" class="w-4 h-4 text-gray-400" />
        <span class="text-gray-900 dark:text-white font-medium truncate">{{ caseStudy?.title || 'Loading...' }}</span>
      </nav>
    </div>
    ...
    <div v-else-if="caseStudy" class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <!-- Key Results -->
      <div v-if="getResults(caseStudy).length > 0" class="grid grid-cols-2 md:grid-cols-3 gap-6 mb-12">
      ...
      <!-- Challenge Section -->
      <div v-if="caseStudy.challenge" class="mb-12">
      ...
      <!-- Solution Section -->
      <div v-if="caseStudy.solution" class="mb-12">
        <h2 class="text-3xl font-bold mb-6 flex items-center">
      ...
      <!-- Technology Stack -->
      <div v-if="caseStudy.technologies && caseStudy.technologies.length > 0" class="mb-12">
        <h2 class="text-3xl font-bold mb-6 flex items-center">
      ...
      <div v-if="relatedCaseStudies && relatedCaseStudies.length > 0" class="mt-16">
        <h2 class="text-3xl font-bold mb-8 flex items-center">
          <Icon name="lucide:briefcase" class="w-8 h-8 text-orange-500 mr-3" />
          Related Case Studies
        </h2>
        <div class="flex gap-6 overflow-x-auto pb-4 scrollbar-hide" style="scroll-snap-type: x mandatory">
          <NuxtLink
              ...
              class="flex-shrink-0 w-80 group bg-white dark:bg-gray-900 rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-800 hover:border-orange-500 dark:hover:border-orange-500 shadow-lg hover:shadow-xl transition-all duration-500"
              style="scroll-snap-align: start"
          >
            <div class="relative h-48 bg-gradient-to-br from-orange-500 via-purple-600 to-pink-600 flex-shrink-0">
              <img
                  v-if="related.image"
                  :src="related.image"
                  :alt="related.title"
                  class="w-full h-full object-cover"
              />
              <Icon v-else name="lucide:briefcase" class="w-16 h-16 text-white/30 absolute inset-0 m-auto" />
              <div class="absolute top-3 right-3">
                <span class="px-2 py-1 bg-white/20 backdrop-blur-sm border border-white/30 rounded text-white text-xs font-bold">
                  {{ related.industry || 'Case Study' }}
                </span>
              </div>
            </div>
            <div class="p-5 flex-shrink-0">
              <h3 class="font-bold text-gray-900 dark:text-white group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors line-clamp-2">
                {{ related.title }}
              </h3>
              ...
              <div class="flex items-center justify-between mt-4">
                ...
                <Icon name="lucide:arrow-right" class="w-4 h-4 text-orange-600 dark:text-orange-400" />
              </div>
            </div>
          </NuxtLink>
        </div>
