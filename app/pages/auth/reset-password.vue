  ...
  <div class="min-h-screen bg-titanium-950 text-white flex items-center justify-center px-4 py-12">
    <div class="w-full max-w-md bg-titanium-900/90 border border-titanium-800 rounded-3xl p-8 shadow-2xl backdrop-blur-xl">
      <div class="text-center mb-6">
        ...
        <h1 class="text-3xl font-bold mt-2">Reset password</h1>
        <p class="text-titanium-400 text-sm mt-1">Set a new password using the token you received.</p>
      </div>
      ...
      <form @submit.prevent="handleReset" class="space-y-4">
        ...
        <div>
          <label class="block text-xs uppercase tracking-[0.3em] text-titanium-400">Reset token</label>
          <input
            v-model="token"
            type="text"
            required
        ...
        <div>
          <label class="block text-xs uppercase tracking-[0.3em] text-titanium-400">New password</label>
          <input
            v-model="password"
            type="password"
            required
...
const handleReset = async () => {
  ...
  try {
    await useSanctumFetch(`${config.public.apiBase}/api/auth/password/reset`, {
      method: 'POST',
      body: {
        email: email.value,
        token: token.value,
    ...
    error.value = err?.data?.message || 'Unable to reset password.'
  } finally {
    loading.value = false
  }
}
